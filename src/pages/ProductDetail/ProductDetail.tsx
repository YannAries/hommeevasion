...
import {
  ...
  KeyInfo,
  Reviews,
} from "../../components/molecules/ProductDetail";
...
const ProductDetail = ({  }: Props) => {
  ...
  useEffect(() => {
    const getProductData = () => {
      ...
      api
        ...
        .catch((error) => {
          console.error(
            "ProudctDetails.tsx.getProudctData() : " + error.toString()
          );
          setLoading(false);
        });
    };
  ...
  if (!productId) {
    return <NotFound404 message="Article non trouvé" />;
  }

  if (!product && !loading) return <NotFound404 message="Article non trouvé" />;
  return (
